...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        sub_node = CategorySerializer(
            data={"category_name": "sub_category", "parent": root_category.id}
        )
        sub_node.is_valid()
        sub_node.save()

        sub_category = Category.objects.get(category_name="sub_category")
        last_node = CategorySerializer(
            data={"category_name": "last_category", "parent": sub_category.id}
        )
        last_node.is_valid()
        last_node.save()
