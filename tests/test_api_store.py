...
class TestCategory:
    ...
    def test_admin_create(self):
        ...
        test_user = User.objects.create(
            email="admin@sarawan.ru", password=123456, is_superuser=True
        )
        request = factory.post("/category/", {"category_name": "test_1"}, format="json")
        force_authenticate(request, user=test_user)
        response = view(request)
...
class TestProduct:
    ...
    def test_create(self):
        ...
        request = factory.post(
            "/product/",
            {"product_name": "test", "category": category.pk},
            format="json",
        )
        force_authenticate(request, user=test_user)
        response = view(request)
    ...
    def test_update(self):
        ...
        category = Category.objects.create(category_name="test_1")
        product = Product.objects.create(product_name="test_1", category=category)
        request_update = factory.put(
            "/product/", {"product_name": "test_2", "category": category}
        )
